...
void automaticSeparate(char*);
void manualSeparate(char*);
...
int main(void)
{
	...
	while (!stop){
		...
		strcat(fileName, " Spec Check.txt");
	...
	if(choice1=='2')
		automaticSeparate(specName);
	else if(choice1=='1')
		manualSeparate(specName);
	else printf("Error.\n");	//bug check
...
void automaticSeparate(char *otherName)
{
	...
	for(i=0;tableArray[i].count!=0;i++)
	{
...
void manualSeparate(char *fileName)
{
	//declare & initialize variables, pointers, and arrays
	char choice;
	FILE *table,*outfile;
...
//	if it returns -1, it means that the section does not appear. Otherwise it will return
...
void separateSections(section_t *tableArray, char *specName, int *skipArray)
{
	...
	//BUG FIX: extraneous, randomly assigned skipArrays causing program to not delete certain files
	for(j=index;j<SECTNUM;j++)
		skipArray[j]=-1;
    ...
    *swncount=0;
    ...
    while(fscanf(ifile, "%c", &character)!=EOF)
    {
        if(character=='/')
        {
            ...
            if(character=='/')
            {
                fscanf(ifile, "%s", string);    //make sure not followed by www. or WWW.
                if(!((string[0]=='w' && string [1]=='w' && string[2]=='w' && string[3]=='.') || (string[0]=='W' && string [1]=='W' && string[2]=='W' && string[3]=='.')))
                {
                    (*slashcount)++;
    }   }   }   }
...
//	BUG FIX: using if(fopen(...)!=NULL) opens the file, and you can't close it. this doesn't
...
void clearFiles(section_t *tableArray, int *skipArray, char *tableName, char *bodyName)
{
	...
	for(i=0; tableArray[i].count!=0;i++)
	{
		if(!(i==skipArray[index] && skipArray[index]!=-1))
		{
