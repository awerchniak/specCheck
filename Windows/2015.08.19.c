...
void manualSeparate(void)
{
	...
	//tell clearFiles not to do anything
	outfile=fopen("sections.txt", "w");
		fprintf(outfile, "0");
	fclose(outfile);
...
void separateSections(section_t *tableArray, char *specName, int *skipArray)
{
	...
	//set all entries in skipArray to 0;
	for(i=0;i<SECTNUM;i++)
		skipArray[i]=0;
...
/* clearFiles */
//	name misleading, it does this on Macs
//	for windows, this just creates a saved file that clearFiles then goes into and reads
//	to delete all of the extra files.
void clearFiles(section_t *tableArray, int *skipArray, char *tableName, char *bodyName)
{
	...
	//count the number of sections in the table
	for(tableCount=0;tableArray[tableCount+1].count!=0;tableCount++);
	
	//write a file called "sections.txt"
	outfile = fopen("sections.txt", "w");
	
	//tell clearFiles to do its thing
	fprintf(outfile, "1\n");
	
	//print number of sections (first thing clearFiles.exe will scan)
	fprintf(outfile, "%d\n",tableCount);
	
	//print all of the section names
	for(i=1; tableArray[i].count!=0;i++)
	{
		fprintf(outfile, "%s\n", tableArray[i].shortName);
	}
	
	//print the indices of any sections that don't appear
	//	tableArray starts at 1, but the array in clearFiles is a normal one
	for(i=0; skipArray[i]!=0; i++)
	{
		skipArray[i]--;
		fprintf(outfile, "%d\n", skipArray[i]);
	}
